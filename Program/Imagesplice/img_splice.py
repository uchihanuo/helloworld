...
def main():
    ...
    img_list = [cv.imread((os.path.join(os.path.abspath('.'), img_file)), cv.IMREAD_GRAYSCALE)
    ...
    img_list = [cv.imread((os.path.join(os.path.abspath('.'), img_file)), cv.IMREAD_UNCHANGED)
                for img_file in os.listdir('.') if img_file.endswith('.PNG')]
