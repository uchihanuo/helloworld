...
def main():
    ...
    img_list = [np.array(img_file.convert('L')) for img_file in img_pil]
    ...
    img_list = [np.array(img_file) for img_file in img_pil]
    ...
    # Image.fromarray(img).save(os.path.join(os.path.abspath('.'),'IMG_' +time.strftime('%Y%m%d%H%M') +'.PNG'))
