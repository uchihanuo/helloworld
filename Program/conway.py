...
def randomGrid(N):
    """returns a grid of N*N random valuse"""
...
def update(frameNum, img, grid, N):
    ...
    for i in range(N):
        for j in range(N):
            # compute 8-neighbor sum using toroidal boundary conditions x and y warp around
            # so that the simulation takes place on a toroidal surface
            total = int((grid[(i - 1) % N, (j - 1) % N]) + (grid[(i - 1) % N, (j + 1) % N]) +
...
def main():
    ...
    if args.glider:
        ...
        # population grid with random on/off - more off than on
        grid = randomGrid(N)
