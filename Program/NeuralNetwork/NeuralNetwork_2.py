...
class NEURALNETWORK:
    ...
    def query(self, inputs_list, weight):
        # convert inputs list to 2d array
        inputs = np.array(inputs_list, ndmin=2).T
        with open(weight, 'rb') as f:
            weight_dict = pickle.load(f)
            # calculate signals into hidden layer
            hidden_inputs = np.dot(weight_dict['wih'], inputs)
            # calculate signals emerging from hidden layer
            hidden_outputs = self.activation_function(hidden_inputs)

            # calculate signals into final outputs layer
            final_inputs = np.dot(weight_dict['who'], hidden_outputs)
            # calculate signals emerging from final outputs layer
            final_outputs = self.activation_function(final_inputs)
        return final_outputs

    # backquery the neural network
    def backquery(self, targets_list, weight):
        ...
        with open(weight, 'rb') as f:
            ...
            hidden_outputs = np.dot(weight_dict['who'].T, final_inputs)
            ...
            inputs = np.dot(weight_dict['wih'].T, hidden_inputs)
            # scale them back to 0.01 to 0.99
    ...
    # to save network weight
    def saveweight(self, file_path):
        network_weight = {'wih': self.wih, 'who': self.who}
        with open(file_path, 'wb') as f:
            pickle.dump(network_weight, f)
        return print('Network weight file has saved.')
...
for record in querying_data_list:
    ...
    querying_outputs = n.query(querying_inputs, weight_file)
...
for i in range(output_nodes):
    ...
    target[i] = 0.99
    image_data = n.backquery(target, weight_file)
